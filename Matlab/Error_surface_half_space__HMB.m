...
for j=5
    ...
    Origlimx=min(Tabledata.XCoordinate);
    OrigliMx=max(Tabledata.XCoordinate);
    Origlimy=min(Tabledata.YCoordinate);
    OrigliMy=max(Tabledata.YCoordinate);
...
Detector_position_vec=[Tabledata.XCoordinate,Tabledata.YCoordinate];
    ...
    zx=linspace(Origlimx,OrigliMx,5e2);
    zy=linspace(Origlimy,OrigliMy,5e2);
...
save(['./Roost_data/',Places{j},'/',Names{j},'_error_surface_half.mat'])
end